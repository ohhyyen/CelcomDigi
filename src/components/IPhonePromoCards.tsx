...
interface IPhonePromoCardsProps {
  onSelectIPhone: (iphone: { name: string; image: string; price: string; ram: string; colors: string[]; storage: string[]; camera: string; }) => void;
  onLearnMore: (iphone: { name: string; image: string; price: string; ram: string; colors: string[]; storage: string[]; camera: string; }) => void;
}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
      {iPhones.map((iphone, index) => (
        <div key={index} className="bg-white rounded-lg shadow-md p-6 text-center">
          ...
          <p className="text-sm text-gray-500 mb-1">Storan: {iphone.storage.join(', ')}</p>
          <p className="text-sm text-gray-500 mb-1">Kamera: {iphone.camera}</p>
